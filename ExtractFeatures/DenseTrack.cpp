...
int DenseTrack(int argc, char** argv)
{
    ...
    if(flag) {
        if (start_frame >= seqInfo.length)
            return -1;
        if (end_frame >= seqInfo.length)
            end_frame = seqInfo.length - 1;
        seqInfo.length = end_frame - start_frame + 1;
        if (track_length >= seqInfo.length)
//            track_length = seqInfo.length - 1;
            return -1;
    }
